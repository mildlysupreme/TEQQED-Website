function runGSAP() {
  const wrapper = document.querySelector(".projects-list");
  const boxes = gsap.utils.toArray(".projects-item");
  console.clear();
...
function horizontalLoop(items, config) {
  ...
  if (window.matchMedia("(min-width: 992px)").matches) {
    ...
    function resetHeaders() {
      const headers = document.querySelectorAll(".h1.h1--marquee");
    ...
    projectItems.forEach((item) => {
      ...
      const previewTextBlocks = document.querySelectorAll(
        ".projects-preview-item .text-block"
      );

      previewTextBlocks.forEach((previewTextBlock) => {
        if (previewTextBlock.textContent === textBlock) {
          const previewItem = previewTextBlock.closest(
          ...
          item.addEventListener("mouseenter", () => {
            ...
            previewItem.style.transition = "opacity 0.1s linear";

            headerElements.forEach((header) => {
              if (header !== item.querySelector(".h1.h1--marquee")) {
                header.style.color = "#ffffff";
                header.style.opacity = "1";
                header.style.transition =
                  "color 0.1s ease-in-out, opacity 0.1s ease-in-out";
            ...
            const otherProjectItems = document.querySelectorAll(
              ".projects-item:not(:hover)"
            );
            otherProjectItems.forEach((otherItem) => {
              const otherHeaderElements = otherItem.querySelectorAll(
                ".h1.h1--marquee"
              );
              otherHeaderElements.forEach((otherHeader) => {
                otherHeader.style.color = "#ffffff";
                otherHeader.style.opacity = "1";
                otherHeader.style.transition =
                  "color 0.1s ease-in-out, opacity 0.1s ease-in-out";
              });
            });
          });
          ...
          item.addEventListener("mouseleave", () => {
            previewItem.style.opacity = "0";
            previewItem.style.transition = "opacity 0.3s linear";
            setTimeout(() => {
              previewItem.style.opacity = "0";
            }, 200);
